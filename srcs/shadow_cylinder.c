...
bool	disk_check(t_vector base, t_ray *shadow, t_cylinder *cylinder, float light_dist)
{
	float		denom;
	float		t;
	t_vector	axis;
	...
	if (t < 0 || t * t > light_dist)
		return (false);
	shadow->hit_point = vector_add(shadow->origin, vector_multi(t, shadow->direction));
	...
	float light_dist)
{
	...
	while (params->cylinder[++i])
	{
		...
		base = vector_sub(pos_to_vector(params->cylinder[i]->pos), vector_multi(params->cylinder[i]->h / 2, axis));
		top = vector_add(pos_to_vector(params->cylinder[i]->pos), vector_multi(params->cylinder[i]->h / 2, axis));
	...
	float light_dist)
{
	...
	while (params->cylinder[++i])
	{
		...
		t.d_perp = vector_sub(shadow->direction, vector_multi(vector_dot(shadow->direction, t.axis), t.axis));
